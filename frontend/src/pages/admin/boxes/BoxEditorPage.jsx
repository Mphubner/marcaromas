...
export default function BoxEditorPage() {
  ...
  // Carregar box existente
  const { isLoading: isLoadingBox } = useQuery({
    queryKey: ['box-edit', id],
    queryFn: async () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async (data) => {
      const response = await api.patch(`/boxes/${id}`, data);
      return response.data;
    },
    onSuccess: () => {
      toast.success('Box atualizada com sucesso!');
      queryClient.invalidateQueries(['admin-boxes']);
      queryClient.invalidateQueries(['box-edit', id]);
      navigate(`/admin/boxes/${id}`);
    },
    onError: (error) => {
      toast.error(error.response?.data?.message || 'Erro ao atualizar box');
    }
  });
  ...
  if (isLoadingBox) {
    return (
      <div className="p-8 max-w-5xl mx-auto">
        <p className="text-center text-gray-500">Carregando box...</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-6">
        <Button
          variant="outline"
          onClick={() => navigate(`/admin/boxes/${id}`)}
        ...
        <div>
          <h2 className="text-3xl font-bold text-[#2C2419]">Editar Box</h2>
          <p className="text-gray-600 mt-1">{formData.theme || 'Box #' + id}</p>
        </div>
      </div>
